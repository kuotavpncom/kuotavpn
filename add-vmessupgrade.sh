...
pathku=$(cat /etc/xray/path/vmess_upgrade_path)
...
echo -e "━━━━━━━━━━━━━━━━━━━━━"
echo -e "LINK HTTPUPGRADE TLS : ${vmesslink1}"
echo -e "━━━━━━━━━━━━━━━━━━━━━"
...
sed -i 's/LINK HTTPUPGRADE TLS : \(.*\)/LINK HTTPUPGRADE TLS : <code>\1<\/code>/g' /tmp/created-vmessupgrade.log
sed -i 's/LINK HTTPUPGRADE NONE-TLS : \(.*\)/LINK HTTPUPGRADE NONE-TLS : <code>\1<\/code>/g' /tmp/created-vmessupgrade.log
KEY=$(cat /etc/william/profile/private/key 2>/dev/null)
CHATID=$(cat /etc/william/profile/private/chatid 2>/dev/null)
message=$(cat /tmp/created-vmessupgrade.log)
curl -s -X POST https://api.telegram.org/bot${KEY}/sendMessage -F chat_id="${CHATID}" -F text="
$message" -F parse_mode=html > /dev/null 2>&1
rm -rf /tmp/created-vmessupgrade.log
while true; do
    output=$(xv $user $uuid vmess-upgrade 2>&1)
    if [[ -z $output ]]; then
        echo ""
        echo "Success"
        break
...
while true; do
    output=$(xvn $user $uuid vmess-upgrade 2>&1)
    if [[ -z $output ]]; then
        echo "Success"
        break
    fi
