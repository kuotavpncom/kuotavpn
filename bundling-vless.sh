...
echo -e "Limit IP VLWS : Not Active"
else
if [ "$ceklimit_ip" == "0" ]; then
echo -e "Limit IP VLWS : No Limit IP VLWS Login"
else
echo -e "Limit IP VLWS : $limit_ip"
fi
fi
...
sed -i 's/LINK gRPC TLS : \(.*\)/LINK gRPC TLS : <code>\1<\/code>/g' /tmp/created-vless.log
sed -i 's/LINK TCP XTLS : \(.*\)/LINK TCP XTLS : <code>\1<\/code>/g' /tmp/created-vless.log
