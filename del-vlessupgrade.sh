...
sed -i "/^### $user $exp $protocol1 /,+1d" /usr/local/etc/xray/config.json
sed -i "/^### $user $exp $protocol2 /,+1d" /usr/local/etc/xray/none.json
...
rm -rf /etc/william/limit-xray/vlessupgrade/${user}
clear
echo ""
xd vless-upgrade $user
echo "━━━━━━━━━━━━━━━━━━━━"
