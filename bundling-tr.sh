...
echo -e "Limit IP TRWS : Not Active"
else
if [ "$ceklimit_ip" == "0" ]; then
echo -e "Limit IP TRWS : No Limit IP TRWS Login"
else
echo -e "Limit IP TRWS : $limit_ip"
fi
fi
...
sed -i 's/LINK HTTPUpgrade TLS : \(.*\)/LINK HTTPUpgrade TLS : <code>\1<\/code>/g' /tmp/created-trws.log
sed -i 's/LINK gRPC TLS : \(.*\)/LINK gRPC TLS : <code>\1<\/code>/g' /tmp/created-trws.log
sed -i 's/LINK TCP TLS : \(.*\)/LINK TCP TLS : <code>\1<\/code>/g' /tmp/created-trws.log
