...
echo -e "----------------------------------"
echo -e " [10]  CHANGE UUID/PASS FOR VMESS HTTPUPGRADE"
echo -e " [11]  CHANGE UUID/PASS FOR VLESS HTTPUPGRADE"
...
validasiuuid=$(grep -rw "/usr/local/etc/xray/config.json" -e "$uuidnew" | awk {'print $2'} | sed 's/"level"://g' | tr -d '"' | tr -d ',' | sort -u)
if [ "$uuidnew" == "$validasiuuid" ]; then
echo -e "${red}uuid/pass sudah digunakan${NC}"
echo -e "${red}uuid/pass already used${NC}"
exit 0
fi
sed -i "s#${uuidold}#${uuidnew}#g" /usr/local/etc/xray/config.json
clear
...
if [[ $protocol1 = "VmessUPGRADE-TLS" ]]; then
NUMBER_OF_CLIENTS=$(grep -c -E "$protocol1 " "/usr/local/etc/xray/config.json")
        if [[ ${NUMBER_OF_CLIENTS} == '0' ]]; then
                echo ""
                echo "You have no existing clients !"
...
validasiuuid=$(grep -rw "/usr/local/etc/xray/config.json" -e "$uuidnew" | awk {'print $2'} | sed 's/security://g' | tr -d '"' | tr -d ',' | sort -u)
if [ "$uuidnew" == "$validasiuuid" ]; then
  echo -e "${red}uuid sudah digunakan${NC}"
  echo -e "${red}uuid already used${NC}"
  exit 0
...
;;
11)
protocol1=$(grep -E -w "VlessUPGRADE-TLS" "/usr/local/etc/xray/config.json" | cut -d ' ' -f 4-4 | head -1)
protocol2=$(grep -E -w "VlessUPGRADE-NTLS" "/usr/local/etc/xray/none.json" | cut -d ' ' -f 4-4 | head -1)
if [[ $protocol1 = "VlessUPGRADE-TLS" ]]; then
NUMBER_OF_CLIENTS=$(grep -c -E "$protocol1 " "/usr/local/etc/xray/config.json")
        if [[ ${NUMBER_OF_CLIENTS} == '0' ]]; then
                echo ""
                echo "You have no existing clients !"
...
;;
12)
protocol=$(grep -E "TrojanUPGRADE-TLS" "/usr/local/etc/xray/config.json" | cut -d ' ' -f 4-4 | head -1)
if [[ $protocol = "TrojanUPGRADE-TLS" ]]; then
NUMBER_OF_CLIENTS=$(grep -c -E "$protocol " "/usr/local/etc/xray/config.json")
	if [[ ${NUMBER_OF_CLIENTS} == '0' ]]; then
		echo ""
		echo "You have no existing clients!"
