...
pathku=$(cat /etc/xray/path/trojan_upgrade_path)
...
protocol1=$(grep -E -w "TrojanUPGRADE-TLS" "/usr/local/etc/xray/config.json" | cut -d ' ' -f 4-4 | head -1)
if [[ $protocol1 = "TrojanUPGRADE-TLS" ]];
then
echo "found"
else
...
uuid=$(cat /usr/local/etc/xray/config.json | grep -w "$user" | awk {'print $2'} | tail -n 1 | tr -d '"' | sed 's/security://g' | tr -d ',')
trojanlink="trojan://${uuid}@${domain}:${tls}?path=${pathku}&security=tls&host=${domain}&type=httpupgrade&sni=${domain}#${user}"
clear
echo -e ""
echo -e "━━━━━━━━━━━━━━━━━━━━━"
echo -e "━━[XRAY/TROJAN_UPGRADE]━━"
echo -e "━━━━━━━━━━━━━━━━━━━━━"
echo -e "Remarks : ${user}"
