...
old=$(cat /etc/xray/path/vmess_upgrade_path)
echo -e "━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo -e " ${green}⇱ CHANGE PATH VMESS HTTPUPGRADE ⇲ ${NC}"
echo -e "━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
echo -e "━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo -e "Your Current Path : ${green}$old${NC}"
echo -e "━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo -e "Please Enter A New Path For Vmess HTTPUpgrade"
read -p "New Path : " new
if [ -z $new ]; then
...
invalidChars="/"
if [[ $new = *["$invalidChars"]* ]]; then
    echo ""
    echo "Jangan Menggunakan '/'"
    echo "Don't Use '/'"
    exit 0
fi
if [[ $new =~ ^[0-9]+$ ]]; then
    echo "Path tidak boleh hanya berisi angka"
    exit 0
fi
if [[ ${#new} -gt 15 ]] ; then
    echo "Panjang Path tidak boleh lebih dari 15 karakter"
    exit 0
fi
if [[ $new =~ [[:space:]] ]]; then
    echo "Path tidak boleh mengandung spasi"
    exit 0
fi
cek="vmess"
if [ "$new" = "$cek" ]; then
...
escaped_old=$(echo "$old" | sed 's#/##')
sed -i "s#\([^#]\)\b${escaped_old}\b#\1${new}#g" /usr/local/etc/xray/config.json
sed -i "s#\([^#]\)\b${escaped_old}\b#\1${new}#g" /usr/local/etc/xray/none.json
sed -i "s#\([^#]\)\b${escaped_old}\b#\1${new}#g" /etc/nginx/conf.d/vmnone.conf
sed -i "s#\([^#]\)\b${escaped_old}\b#\1${new}#g" /usr/local/etc/xray/will666.json
sed -i "s#\([^#]\)\b${escaped_old}\b#\1${new}#g" /etc/xray/path/vmess_upgrade_path
echo -e ""
