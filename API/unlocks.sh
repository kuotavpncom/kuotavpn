...
echo -e "No SSH Account Got Banned/Locked !"
exit
fi
...
ngecek_banned=$(cat /etc/william/udp/listbanned-ssh.conf | grep -w "$user" | sort -u)
if [[ -z "$ngecek_banned" ]]; then
echo -e "user ${user} not found in database locked/banned"
exit
fi
