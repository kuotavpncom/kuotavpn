...
cekuser=$(cat /var/lib/premium-script/data-user-l2tp | grep -w "$user" | cut -d ' ' -f 2)
if [[ -z "$cekuser" ]]; then
    echo "user $user not found !"
    exit
fi
exp=$(cat /var/lib/premium-script/data-user-l2tp | grep -w "$user" | cut -d ' ' -f 3)
now=$(date +%Y-%m-%d)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
