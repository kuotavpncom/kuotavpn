...
cek_lock=$(grep '"email": "'"$user"'"' /usr/local/etc/xray/config.json | tail -n 1 | awk '{print $1}')
if [[ "$cek_lock" = "#lock" ]]; then
...
# Menambahkan #lock di awal baris yang mengandung email user
sed -i '/"email": "'"$user"'"/s/^/#lock /' /usr/local/etc/xray/config.json
sed -i '/"email": "'"$user"'"/s/^/#lock /' /usr/local/etc/xray/none.json
echo -e "━━━━━━━━━━━━━━━━━━━━"
