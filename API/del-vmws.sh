...
# Perform the necessary deletion operations
protocol1=$(grep -E -w "VmessWS-TLS" "/usr/local/etc/xray/config.json" | cut -d ' ' -f 4 | head -1)
protocol2=$(grep -E -w "VmessWS-NTLS" "/usr/local/etc/xray/none.json" | cut -d ' ' -f 4 | head -1)
...
sed -i "/^### $user $exp $protocol1 /,+1d" /etc/cf-argo/config.json
