...
sed -i "s/### $user $exp $protocol /### $user $new_exp $protocol /g" /usr/local/etc/xray/config.json
sed -i "s/### $user $exp $protocol /### $user $new_exp $protocol /g" /etc/cf-argo/config.json
else
now=$(date +%Y-%m-%d)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
...
sed -i "s/### $user $exp $protocol /### $user $exp4 $protocol /g" /etc/cf-argo/config.json
fi
